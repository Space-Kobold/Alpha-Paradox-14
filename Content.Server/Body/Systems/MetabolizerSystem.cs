...
namespace Content.Server.Body.Systems
{
    public sealed class MetabolizerSystem : EntitySystem
    {
        ...
        private void TryMetabolize(Entity<MetabolizerComponent, OrganComponent?, SolutionContainerManagerComponent?> ent)
        {
            ...
            {
                if (!_chemRegistry.TryIndexReagent(reagent.Prototype, out var reagentDef))
                    continue;

                var mostToRemove = FixedPoint2.Zero;
                if (reagentDef.Metabolisms is null)
                {
                ...
                {
                    if (!reagentDef.Metabolisms.TryGetValue(group.Id, out var entry))
                        continue;
                    ...
                    {
                        if (!reagentDef.WorksOnTheDead && _mobStateSystem.IsDead(solutionEntityUid.Value, state))
                            continue;
                    }
                    ...
                    foreach (var effect in entry.Effects)
                    {
                        ...
                        if (effect.ShouldLog)
                        {
                            _adminLogger.Add(
                                ...
                                + $" of reagent {reagentDef.LocalizedName:reagent}"
